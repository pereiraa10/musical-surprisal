...
instead of being duplicated inline. PreparedSubject runs the feature_set-
independent pipeline once per subject; both feature_sets below reuse it via
.to_dataset(feature_set, ...).
...
def main():
    ...
    for SUBJECT in config.subjects:
        ...
        for feature_set, feature_keys in config.feature_sets.items():
            ...
            print(f"  {SUBJECT} | {feature_set} [MNE]: "
            ...
            path = res.result_filename(save_dir, SUBJECT, 'mne_ridge', feature_set)
            res.save_result(path, result)

            print(f"  {SUBJECT} | {feature_set}: MNE Ridge mean r = "
